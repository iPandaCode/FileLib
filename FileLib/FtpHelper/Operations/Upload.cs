    ...
    public partial class FtpHelper
    {
        ...
        private void Upload(FileInfoEntity info)
        {
            ...
            try
            {
                ...
                //判断源文件是否更新或者移除
                else if (info.Length != FileHelper.GetFileLength(info.FilePath + "/" + info.FileName))
                {
                    info.ResultCode = ResultCode.SourceError;
                    throw new Exception();
                }
                //判断目标目录是否被移除
                else if (!CheckExistOfRemoteDirectory(_Url + info.NewFilePath))
                {
